...
def add_users():
    users = get('http://localhost:8080/api/v2/users').json()
    next_id = max(([0] + [i['id'] for i in users['users']])) + 1
    user1 = post('http://localhost:8080/api/v2/users', json={
        'name': 'Арсений',
        'surname': 'Шванев',
        'age': 20,
    ...
    user2 = post('http://localhost:8080/api/v2/users', json={
        'name': 'Анна',
        'surname': 'Илина',
        'age': 21,
...
def del_users(*args):
    for i in args:
        response = delete(f'http://localhost:8080/api/v2/users/{i["user"]["id"]}')
...
def test_get_users():
    ...
    response = get('http://localhost:8080/api/v2/users').json()
    assert 'error' not in response
...
def test_get_user():
    ...
    for user in users:
        response = get(f'http://localhost:8080/api/v2/users/{user["user"]["id"]}').json()
        assert 'error' not in response
    ...
    response = get('http://localhost:8080/api/v2/users/0').json()
    assert 'error' in response
    assert response['error'] == 'Not found'

    # Тестирование получение пользователя по строчному id
    response = get('http://localhost:8080/api/v2/users/a').json()
    assert 'error' in response
    assert response['error'] == 'Not found'
...
def test_create_user():
    ...
    users = get('http://localhost:8080/api/v2/users').json()
    next_id = max(([0] + [i['id'] for i in users['users']])) + 1
    user = post('http://localhost:8080/api/v2/users', json={
        'name': 'Арсений',
        'surname': 'Шванев',
        'age': 20,
        'email': f'shwanev.gzed@gmail.com{next_id}',
    ...
    response = post('http://localhost:8080/api/v2/users').json()
...
def test_del_user():
    ...
    response = delete(f'http://localhost:8080/api/v2/users/{users[0]["user"]["id"]}').json()
    assert 'error' not in response
    users_after_del = get('http://localhost:8080/api/v2/users').json()
    assert users[0]["user"]["id"] not in [i['id'] for i in users_after_del['users']]

    # Удаление пользователя с несуществующим id
    response = delete('http://localhost:8080/api/v2/jobs/0').json()
    assert 'error' in response
    assert response['error'] == 'Not found'

    # Удаление пользователя по строчному id
    response = delete('http://localhost:8080/api/v2/jobs/a').json()
    assert 'error' in response
    assert response['error'] == 'Not found'
...
def test_update_user():
    ...
    update_user = put(f'http://localhost:8080/api/v2/users/{users[0]["user"]["id"]}', json={
        'name': 'Обновленное имя'
    }).json()
    assert 'error' not in update_user
    assert update_user['user']['name'] == 'Обновленное имя'
    user = get(f'http://localhost:8080/api/v2/users/{users[0]["user"]["id"]}').json()
    assert user['user']['name'] == 'Обновленное имя'

    # Обновление пользователя с несуществующим id
    update_user = put('http://localhost:8080/api/v2/users/0', json={
        'name': 'Обновленное имя'
    }).json()
    ...
    update_user = put('http://localhost:8080/api/v2/users/a', json={
        'name': 'Обновленное имя'
    }).json()
    assert 'error' in update_user
    assert update_user['error'] == 'Not found'
