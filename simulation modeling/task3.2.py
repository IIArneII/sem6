...
if __name__ == '__main__':
    ...
    zi = [sum(i) - 6 for i in xi]

    m_o = sum(zi) / n
    d_o = sum([(i - m_o) ** 2 for i in zi]) / n

    print('Погрешность матожидания:', math.fabs(3 - m_o))
    print('Погрешность дисперсии:', math.fabs(0.25 - math.sqrt(d_o)))

    xi = [[rand() for j in range(2)] for i in range(n)]

    for i in range(len(xi)):
        print(i, xi[i], "   ", math.sqrt(-2 * math.log(xi[i][0])))

    zi = [math.sqrt(-2 * math.log(i[0])) * math.cos(2 * math.pi * i[1]) for i in xi]
